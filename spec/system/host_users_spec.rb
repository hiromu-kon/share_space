...
RSpec.describe "HostUsers", type: :system do
  let(:host_user) { create :host_user }

  describe "ユーザー新規登録ページ" do
    before do
      visit new_host_user_registration_path
    end
  ...
  describe "ユーザーログインページ" do
    ...
    before do
      visit new_host_user_session_path
    end
    ...
    it "新規登録のリンクを押すと新規登録ページに遷移すること" do
      ...
      expect(current_path).to eq new_host_user_registration_path
    end

    it "ログアウトするとCookieがnilになること" do
      check "ログイン情報を記憶する"
      log_in_as host_user
      delete destroy_host_user_session_path
